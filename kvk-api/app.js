...
const endpoint =
  process.env.KVK_API_ENDPOINT || "/api/v2/testprofile/companies";
...
function transformData(kvkData) {
  const {
    registrationDate,
    foundationDate,
    employees,
...
async function redisFetch(req, res, next) {
  ...
  if (cacheval) {
    res.json(transformData(JSON.parse(cacheval)));
  } else {
    next();
  }
}
